<template>
  <div id="cate-list">
    ...
    <el-table :data="level" border style="width: 100%">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button size="mini" @click="$router.push(`/system/cateUpdate/${scope.row._id}`)"
            >编辑</el-button
          >
          <el-button size="mini" type="danger" @click="deleteCate(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="back">
      <el-button type="primary" @click="fetchOne" v-if="showBackBtn" size="small"
        >返回上级</el-button
      >
    </div>
  </div>
